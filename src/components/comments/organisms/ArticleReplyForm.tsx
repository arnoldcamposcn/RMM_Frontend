...
interface ArticleReplyFormProps extends CommentFormProps {
  parentAuthor: string;
  onCancel: () => void;
  level?: number; // Para el nivel de anidamiento
}

export const ArticleReplyForm: React.FC<ArticleReplyFormProps> = ({
  parentAuthor,
  onCancel,
  level = 1,
  ...commentFormProps
  ...
  return (
    <div className={`${indentLevel} ${borderStyle} pl-4 mt-3`}>
      <div className={`${bgStyle} rounded-lg p-4`}>
        ...
        <ArticleCommentForm {...commentFormProps} />
        <button
          onClick={onCancel}
