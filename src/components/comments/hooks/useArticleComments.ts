...
} from '../../../services/articles/article-comments.service';
...
export const useArticleComments = (articleId: number) => {
  const { data: comments, loading, error, refetch } = useFetch<ArticlePostComment[]>(
  ...
  const isCurrentUserAuthor = (comment: ArticlePostComment): boolean => {
    if (!currentUser) return false;
    return currentUser.usuario_unico === comment.autor.usuario_unico;
  };

  // Función para iniciar edición
  const handleStartEdit = (comment: ArticlePostComment) => {
  ...
  const organizeComments = (comments: ArticlePostComment[]): { parent: ArticlePostComment; replies: ArticlePostComment[] }[] => {
  ...
  const flattenComments = (comments: ArticlePostComment[]): ArticlePostComment[] => {
    const flattened: ArticlePostComment[] = [];
    
    const flatten = (commentList: ArticlePostComment[]) => {
